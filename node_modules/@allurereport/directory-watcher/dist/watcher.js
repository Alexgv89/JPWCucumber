...
export const findMatching = async (watchDirectory, existingResults, match, maximumDepth = 5, signal) => {
    const scan = async (directory, remainingDepth, isRoot) => {
        if (signal?.aborted) {
            return false;
        }
        try {
...
const findFiles = async (watchDirectory, existingResults, onNewFile, recursive, maximumDepth = 10, signal) => {
...
const noop = async () => { };
export const watch = (initialCallback, iterationCallback, doneCallback, options = {}) => {
...
export const newFilesInDirectoryWatcher = (directory, onNewFile, options = {}) => {
    const { recursive = true, maximumDepth = 10, ignoreInitial = false, abortController = new AbortController(), ...rest } = options;
    const indexedFiles = new Set();
    const initialCallback = async () => {
        await findFiles(directory, indexedFiles, ignoreInitial ? noop : onNewFile, recursive, maximumDepth, abortController.signal);
    };
...
export const allureResultsDirectoriesWatcher = (directory, update, options = {}) => {
    ...
    const callback = async () => {
        ...
        const completed = await findMatching(directory, currentAllureResults, (dirent) => dirent.isDirectory() && dirent.name === "allure-results", undefined, abortController.signal);
        if (!completed) {
            return;
        }
