...
export const runTests = async (params) => {
    const { allureReport, cwd, command, commandArgs, logs, environmentVariables, environment, withQualityGate, silent, logProcessExit = true, resultsPatterns = [], } = params;
...
export const executeAllureRun = async (params) => {
    const { allureReport, cwd, command, commandArgs, environmentVariables = {}, environment, withQualityGate, silent, logs, ignoreLogs, maxRerun = 0, logProcessExit = true, resultsPatterns = [], } = params;
    ...
    if (!ignoreLogs && testProcessResult?.stdout) {
        ...
        allureReport.realtimeDispatcher.sendGlobalAttachment(stdoutResultFile, formatProcessLogAttachmentName([command, ...commandArgs].join(" "), "stdout"));
    }
    if (!ignoreLogs && testProcessResult?.stderr) {
        ...
        allureReport.realtimeDispatcher.sendGlobalAttachment(stderrResultFile, formatProcessLogAttachmentName([command, ...commandArgs].join(" "), "stderr"));
        if (processFailed) {
            allureReport.realtimeDispatcher.sendGlobalError({
