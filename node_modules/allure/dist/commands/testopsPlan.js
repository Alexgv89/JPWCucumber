...
export class TestOpsPlanCommand extends Command {
    ...
    async execute() {
        ...
        let tests;
        try {
            tests = await client.get(`/api/rs/jobrun/${jobRunId}/plan`, {
...
TestOpsPlanCommand.paths = [["testops-plan"]];
TestOpsPlanCommand.usage = Command.Usage({
    description: "Fetches a testplan.json for the current TestOps job run",
    details: "Reads ALLURE_JOB_RUN_ID and, when it's set, downloads the test cases selected for that job run " +
