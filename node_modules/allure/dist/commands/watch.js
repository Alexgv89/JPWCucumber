...
export class WatchCommand extends Command {
    constructor() {
        ...
        this.preserve = Option.Boolean("--preserve", {
            description: "Don't clear terminal output on the data refresh",
        });
        this.newOnly = Option.Boolean("--new-only", true, {
            description: "Skip whatever test results already exist on disk at startup and only react to results written after the watch has started, instead of ingesting the existing backlog first (default: true). Pass --no-new-only to ingest the existing backlog too",
        });
    }
    async execute() {
        ...
        const perDirectoryWatchers = new Map();
        let isInitialDiscovery = true;
        const onDiscoveryUpdate = async (newDirectories, deletedDirectories) => {
        ...
        abortFunctions.push(async (immediately) => {
        ...
        const terminationSignal = boundedTerminationSignal(signalInfo, 5000);
        const cleanup = (async () => {
...
WatchCommand.usage = Command.Usage({
    ...
    details: "This command watches for changes in Allure Results directories and updates the report in real-time. " +
